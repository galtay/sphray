...
contains
  ...
  subroutine do_skewers()
    ...
    real(r8b) :: Hcnt, HIcnt, nH, NHI
    real(r8b) :: NHItot(Nwrite), raypos(3,Nwrite)
    integer(i8b) :: face(Nwrite), iwrite
    ...
    snaps: do snapn = GV%StartSnapNum, GV%EndSnapNum
       ...
       open(file=trim(GV%OutputDir)//"/NHI.txt",unit=200)
...
100       format(A,I4,6F8.4, 2E14.5)
          do i = 1,globalraylist%nnb
             ...
             NHItot(iwrite) = NHItot(iwrite) + NHI

          end do

          
          face(iwrite) = curface
          raypos(1:3,iwrite) = ray%start(1:3)

          if (mod(rayn,Nwrite)==0) then
             do i = 1,Nwrite
                write(200,"(I,4ES14.4)") face(i), NHItot(i), raypos(:,i)
             end do
          endif
