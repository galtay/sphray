...
module snap_viewer
 ...
 public :: initnbody,display,attachmenu,createsubmenus, &
  IdleFunc2,NormalKey2,starscale,starcentre,starreset,visible
 public :: da
...
subroutine gaslist
  ...
  do i=1,size(psys%par)
     ...
     if(show_temp.EQ.2) then
        temp=log10(psys%par(i)%xHII+1.e-6)
        red=1+temp/3.
        green=-temp/3.
        blue=0
        alpha_c=MAX(0.1,.5+temp/3.)	  
        call glColor4f(red, green, blue,alpha_c)
     endif
...
!    call outputtext(xp1+108._gldouble,yp1-20._gldouble,rs10)
    write(rs3,'(i3)') int(100.*GV%rayn/real(PLAN(GV%CurSnapNum)%SrcRaysForSnap))
...
subroutine viewbodies
 ...
 win=glutCreateWindow("please, don't close me..")
