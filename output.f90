...
! ==========================================================
  ...
  subroutine config_to_ghead(GV, fnum, ghead)
    ...
    if (GV%Comoving) then
       ghead%a = PLAN(GV%CurSnapNum)%ScalefacAtSnap
...
!=======================================
  ...
  subroutine output_total_snap(GV,pars)
  ...
  use particle_system_mod, only: scale_physical_to_comoving  
     ...
     if (GV%Comoving) call scale_physical_to_comoving(PLAN(GV%CurSnapNum)%ScalefacAtSnap, pars)
     ...
     do ifile = 0, GV%ParFilesPerSnap-1
        ...
        call config_to_ghead( GV, ifile, ghead )
     ...
     if (GV%Comoving) call scale_comoving_to_physical(PLAN(GV%CurSnapNum)%ScalefacAtSnap,pars)
...
!=============================================================
  subroutine ion_frac_out(GV,psys,tree)
...
!     real :: ionVnp, ionRnp, StromV, StromR
!     real :: ionizedR_cm, ionizedR_kpc, ionizedV_cm, ionizedV_kpc
...
!    calculate the ionized number, volume, and mass fractions
     Mfrac = mass_weight_ionfrac(psys%par) 
     GV%mwionfrac = Mfrac
...
!========================================================
  ...
  function number_weight_ionfrac(pars) result(numionfrac)
     ...
     real :: numionfrac                             !< ion fraction n weighted
...
!      the loop here.
       numionfrac = 0.0d0
       do i = 1,size(pars)
          numionfrac = numionfrac + pars(i)%xHI 
       end do
...
!========================================================
  ...
  function mass_weight_ionfrac(pars) result(massionfrac)
     type(particle_type), intent(in) :: pars(:) !< particle system
     real :: massionfrac               !< ion fraction m weighted
     real :: masstot
     integer :: i
     ...
     masstot = 0.0
     do i = 1,size(pars)
        massionfrac = massionfrac + pars(i)%mass * pars(i)%xHI
...
!========================================================
  ...
  function vol_weight_ionfrac(pars) result(volionfrac)
     type(particle_type), intent(in) :: pars(:) !< particle system
     real :: volionfrac               !< ion fraction v weighted

     volionfrac = 1.0

   end function vol_weight_ionfrac
