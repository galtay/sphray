...
contains
  ...
  subroutine read_atomic_rates_file(GV)
    ...
    myname = "read_atomic_rates_file"
    crash = .true.

    logfile = trim(GV%OutputDir) // "/" // "atomic_rates.log"
    call open_formatted_file_w(logfile,loglun)

    write(*,'(A,T29,A)') "using rates file: ", trim(GV%AtomicRatesFile)
    inquire(file=GV%AtomicRatesFile,exist=fthere)
    if (.not. fthere) call myerr("cant find rates file",myname,crash)

    call open_formatted_file_r(GV%AtomicRatesFile,lun)
    do i = 1,RateHeaderLines
       read(lun,*)
    end do

    write(loglun,'(A)',advance="no") "reading in the atomic rates header ... "
