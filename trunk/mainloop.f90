...
module mainloop_mod
  ...
  use global_mod, only: psys, globalraylist, tree
  use global_mod, only: OutputTimes, GV, PLAN
...
contains
  ...
  subroutine mainloop()
    ...
    snaps: do snapn = GV%StartSnapNum, GV%EndSnapNum
       ...
       call readin_snapshot(GV,psys%box,MB)
       ...
       if (GV%DoInitialOutput) then
          GV%OutputIndx = 0
          call output_total_snap(GV,psys%par)      
          GV%OutputIndx = 1
       end if
       ...
       !     begin ray tracing 
       !------------------------- 
       src_rays: do rayn = one, PLAN(snapn)%SrcRaysForSnap
          ...
          if( mod(GV%rayn,GV%IonFracOutRays)==0 ) then
             call ion_frac_out(GV, psys, tree )
          end if
          ...
          if ( GV%OutputIndx <= GV%NumTotOuts ) then
             ...
             if ( outmark >= OutputTimes(GV%OutputIndx) ) then
                call output_total_snap(GV,psys%par)
          ...
          if ( snapn == GV%EndSnapNum ) then
             if ( GV%OutputIndx <= GV%NumTotOuts ) then
                if ( GV%src_rayn==PLAN(snapn)%SrcRaysForSnap ) then
